...
// наличные — кассы первыми, безнал — счета по выпискам; внутри — рупиевый счёт
// первым, он же по умолчанию (рупия — валюта учёта, «Касса ($)» по алфавиту
// оказывалась выше и провоцировала ошибку)
function счетаПоКаналу(канал, selectedId, filter) {
    ...
    return FinUtils.accountOptions(selectedId, filter,
        a => ((a.reconciliation_mode === 'cash_count') === наличные ? 2 : 0)
           + (a.currency_code === 'INR' ? 1 : 0));
}
