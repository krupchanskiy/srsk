...
const CATEGORY_GUESS_RULES = [
    ...
    // Названия продуктов — «Продукты» (ВГ, 21.09.2026: «киноа» — это продукты).
    // Слово ищем с начала, чтобы «мука» не находилась в «Мукунда», а «рис» в «рискнули».
    [/(?<![а-яё])(?:продукт|овощ|фрукт|молок|молоч|круп|бакале|киноа|рис(?![а-яё])|мук[аиуе](?![а-яё])|сахар|соль|специ|орех|изюм|чай(?![а-яё])|кофе|дахи|пюре|творог|панир|сыр(?![а-яё])|сливк|сгущ|банан|манго|яблок|ананас|лимон|шпинат|картоф|помидор|томат|огурц|морков|св[её]кл|капуст|баклажан|горох|нут(?![а-яё])|чечевиц|фасол|м[её]д(?![а-яё])|агар|крахмал|ваниль|какао|кокос|финик|чиа|хмель|каркад|сок(?![а-яё])|соки)/, 'dept_food'],
...
function guessPayrollPositionId(text, list) {
    // «…, заказчик Сундарарупа пр.» — это тот, кто заказал покупку, а не получатель
    // оплаты: всё после слова «заказчик» в поиске имени не участвует (ВГ, 21.09.2026)
    const src = (text || '').toLowerCase().replace(/заказчик.*$/s, ' ');
    // Слова про деньги и общие «духовные» слова именем не считаются: иначе «рупий»
    // совпадало с «рупа» из «Сундара рупа дас», а «дас» — с любым «дас»
    const stop = /^(дас|даса|даси|деви|прабху|матаджи|рупи.*|руб.*|долл.*|евро)$/;
    const tokens = str => str.split(/[^а-яёa-z]+/).filter(w => w.length >= 3 && !stop.test(w));
    const common = (x, y) => { let i = 0; while (i < x.length && i < y.length && x[i] === y[i]) i++; return i; };
    const words = tokens(src);
    // длинное имя — совпадение минимум в 5 букв (падежи: Сундара/Сундару);
    // короткое («Уша») — слово той же длины и не короче на одну букву (Уша/Уше/Ушу),
    // чтобы «ушки» не принимались за Ушу
    const hits = list.filter(p => tokens((p.employee_name || '').toLowerCase())
        .some(n => words.some(w => n.length >= 5
            ? common(n, w) >= 5
            : w.length <= n.length && common(n, w) >= n.length - 1)));
