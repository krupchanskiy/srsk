...
async function loadReport() {
    ...
    box.innerHTML = `
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="card bg-base-100 shadow-sm"><div class="card-body py-3">
                <div class="text-xs uppercase opacity-60">${t('fin_participants_count')}</div>
                <div class="text-xl font-semibold">${p.count}</div>
                <div class="text-xs opacity-60">${t('fin_charged')}: ${fmtB(p.charged)} · ${t('fin_paid')}: ${fmtB(p.paid)}</div>
            </div></div>
            <div class="card bg-base-100 shadow-sm"><div class="card-body py-3">
                <div class="text-xs uppercase opacity-60">${t('fin_income')}</div>
                <div class="text-xl font-semibold text-success">${fmtB(tot.income_base)}</div>
            </div></div>
            <div class="card bg-base-100 shadow-sm"><div class="card-body py-3">
                <div class="text-xs uppercase opacity-60">${t('fin_expense')}</div>
                <div class="text-xl font-semibold text-error">${fmtB(tot.expense_base)}</div>
            </div></div>
            <div class="card bg-base-100 shadow-sm"><div class="card-body py-3">
                <div class="text-xs uppercase opacity-60">${t('fin_net')}</div>
                <div class="text-xl font-semibold ${Number(tot.net_base) < 0 ? 'text-error' : ''}">${fmtB(tot.net_base)}</div>
            </div></div>
        </div>
