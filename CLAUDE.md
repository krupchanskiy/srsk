...
## ⚠️ Боевая среда — единственная

Dev-проекта нет: `mymrijdfqeevoaocbzfy` — это живые деньги, живые гости и живой сайт.
Деплой автоматический: push в `main` оказывается на https://in.rupaseva.com через 1–2 минуты,
без ревью и без промежуточной среды.

**Правила, обязательные для любой сессии:**

1. **Сухой прогон перед любой записью в БД.** MCP `execute_sql` коммитит по умолчанию,
   поэтому проверка делается одним вызовом с транзакцией:
   ```sql
   begin;  /* ... запрос ... */  rollback;
   ```
   Посмотреть результат, и только потом выполнять по-настоящему.
2. **Никакого UPDATE/DELETE без WHERE по конкретным id.** Массовые правки данных —
   только после того, как отдельным SELECT показано, сколько строк и каких заденет.
3. **Финансовые данные меняются через RPC `fin_*`, а не прямым SQL.** У них есть
   проверки, идемпотентность и журнал; прямой UPDATE их обходит и ломает сходимость.
   Исключение — точечные исправления данных, и только после сухого прогона.
4. **Перед правкой логики расчётов — измерить влияние.** Воспроизвести старую и новую
   формулу на реальных данных за период и сравнить числа (как делалось для едоков
   и для авансов). Правка расчёта без замера «до/после» не выкладывается.
5. **Проверять роль, а не только себя.** Функции `fin_*` работают под правами
   пользователя; проверка делается симуляцией:
   ```sql
   select set_config('request.jwt.claims', json_build_object('sub','<user_id>','role','authenticated')::text, true);
   ```
6. **Кэш JS.** После правки любого `js/*.js` поднять `?v=N` во **всех** HTML, где он
   подключён (`grep -rl`), иначе у части людей останется старый код.
7. **Проверить, что выложилось.** После push дождаться и убедиться:
   `curl -s https://in.rupaseva.com/<страница> | grep -o '<файл>?v=[0-9]*'`

**Бэкапы.** План Pro: суточные бэкапы, хранение 7 дней. PITR как аддон —
уточнять статус перед рискованными операциями. Восстановление теряет весь день,
поэтому дешевле не ошибиться, чем откатываться.
