...
let isAbKitchenContext = true;
let abKitchenHiddenProductIdsCache = null;
try {
    sessionStorage.setItem(AB_KITCHEN_CONTEXT_KEY, '1');
} catch {
    // Desktop build is always AB Kitchen, even when storage is unavailable.
}
...
const allModules = {
    ...
    crm: {
        ...
        menuConfig: [
            ...
            { id: 'crm_settings', items: [
                ...
                { id: 'crm_task_templates', href: 'crm/task-templates.html' },
    ...
    finance: {
        ...
        menuConfig: [
            ...
            { id: 'fin_inbox', items: [
                { id: 'fin_inbox', href: 'finance/inbox.html' }
            ]},
            ...
            { id: 'fin_reconciliation', items: [
                { id: 'fin_reconciliation', href: 'finance/reconciliation.html' }
            ]},
            { id: 'fin_analytics', items: [
                { id: 'fin_analytics', href: 'finance/analytics.html' }
            ]},
...
// В desktop-сборке регистрируется только кухонный модуль. Остальные определения
// оставлены как импортированный справочный код, но не доступны роутеру и меню.
const modules = { kitchen: allModules.kitchen };
...
const pagePermissions = {
    ...
    'crm/task-templates.html': 'edit_crm_settings',
...
const MODULE_FOLDERS = ['kitchen', 'stock'];
...
async function loadTranslations(retried = false) {
    ...
    const requiredKeys = ['self_accommodation', 'nav_user_management', 'nav_retreat_prasad', 'purchased', 'nav_residents_list', 'nav_prasad', 'timeline_uncovered_title', 'expected_guests', 'timeline_no_retreat', 'fin_split_dept', 'fin_refine', 'fin_refine_spent'];
