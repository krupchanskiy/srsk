-- 471. Заявка за члена семьи с общим телефоном или почтой
--
-- Марина Алещик отправила заявку, а на дочь форма ответила «вы уже подали
-- заявку»: дочь ввела мамин телефон, функция нашла по нему Марину и сочла
-- заявку дублем. Семей с общим телефоном в базе 23 — это норма, не ошибка.
-- Теперь телефон или почта считаются совпадением только вместе с именем
-- (обычным или духовным); иначе это другой человек со своей карточкой.
...
begin
  ...
  if v_phone is not null then
    select id into v_person from vaishnavas
     where phone = v_phone and not is_deleted and (
           lower(first_name) = lower(v_first)
        or (v_spiritual is not null and lower(spiritual_name) = lower(v_spiritual)))
     limit 1;
  end if;
  if v_person is null and v_email is not null then
    select id into v_person from vaishnavas
     where lower(email) = v_email and not is_deleted and (
           lower(first_name) = lower(v_first)
        or (v_spiritual is not null and lower(spiritual_name) = lower(v_spiritual)))
     limit 1;
  end if;
