-- Себестоимость → Накладные: у оценки зарплаты — откуда она взята (ВГ 25.09):
-- estimate_basis = 'salary' (оклад должности) | 'accrual:ГГГГ-ММ' (последнее начисление). Суммы без изменений.
...
create function public.fin_kitchen_payroll(p_from date, p_to date)
 returns table(month date, position_title text, amount numeric, currency_code text, source text,
               vaishnava_id uuid, person_name text, estimate_basis text)
 language plpgsql
...
begin
  ...
  return query
    ...
    select mo.m, pos.position_title,
           ...
           case when acc.position_id is not null then null
                when pos.salary_amount is not null then 'salary'
                else 'accrual:' || to_char(est.last_period, 'YYYY-MM') end
      from months mo
      cross join pos
      left join vaishnavas v on v.id = pos.vaishnava_id
      left join acc on acc.position_id = pos.id and acc.m = mo.m
      left join lateral (
        select coalesce(pos.salary_amount, la.amount) as amount, la.period as last_period
          from (select 1) one
          left join lateral (select a2.amount, a2.period from fin_payroll_accruals a2
                              where a2.position_id = pos.id and a2.period < mo.m
                              order by a2.period desc limit 1) la on true
      ) est on true
