-- =============================================================
-- Черновики из чата: необязательная привязка строки к зарплатной
-- ведомости (ВГ, 16.09.2026).
--
-- Кейс: один департамент платит зарплату сотруднику, числящемуся в
-- другом («Завод» выдал «Зелёным» зарплату Евгении). Перевод между
-- департаментами + расход получателя — уже штатный путь tg_post_draft
-- («уже потрачено» / разбивка с департаментом-получателем). Не хватало
-- одного: способа сказать «этот расход — ещё и выплата такому-то
-- сотруднику в ведомости», не создавая для этого отдельного перевода
-- поверх уже существующего (иначе — тот же дубль, что был с Ранганатхом).
--
-- Решение: необязательное поле payroll_position_id в строке разбивки.
-- Если задано — после того как fin_create_expense создаёт расход,
-- посадочная проводка этой строки (её id детерминирован из id заявки —
-- fin_private_child_uuid) помечается в fin_payroll_payments точно так
-- же, как если бы её создала сама ведомость через fin_pay_payroll.
-- Работает во всех трёх местах, где эта функция строит строки расхода:
-- свои строки (expense), строки чужого департамента (expense-with-dept),
-- «уже потрачено» после выдачи (transfer-with-rows).
-- =============================================================
