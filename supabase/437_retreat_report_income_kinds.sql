-- Приход участников — тремя строками вместо одной (ВГ, сен 2026): сборы
-- участника изначально бьются на назначения (fin_postings.participant_balance_kind:
-- org_fee/accommodation/meals/general — та же разбивка, что в личном балансе
-- участника), просто раньше вся сумма схлопывалась в одну строку «Оплата
-- от участника». Теперь: «Оргвзнос» и «Проживание» — отдельными строками
-- в самом ретрите, «Питание» — полностью уходит в блок prasad (это и есть
-- приход по прасаду, без него прасад был виден только по донатам). Мелкий
-- остаток без разметки (general/extra) остаётся как «Оплата от участника»
-- с настоящим category_id.
--
-- Строки org_fee/accommodation/meals получают синтетический category_id
-- ('org_fee'/'accommodation'/'meals' вместо uuid) — он используется только
-- как ключ сопоставления в JS (subtractCategoryRows), а не как настоящая
-- категория для dds.html (там таких категорий нет — catTable в
-- fin-analytics.js проверяет формат uuid перед тем, как делать строку
-- кликабельной).
--
-- totals (общий итог ретрита) не меняется — это та же сумма всех проводок
-- объекта, просто теперь для показа она разложена подробнее.
