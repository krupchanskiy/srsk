-- Любое уведомление бота о заявке — ответ на исходное сообщение автора
-- (правило ВГ, 21.09.2026): раньше триггер привязывал ответ к исходному
-- сообщению только для отмен (сторно), а подтверждения проведённых расходов
-- уходили без привязки — в чате над ними Telegram показывал служебную строку
-- «Счёт … создана» (создание темы), и связь с сообщением автора терялась.
--
-- Теперь: если заявка пришла из ТОГО ЖЕ чата, куда идёт уведомление, — reply_to
-- ставится всегда. Если уведомление уходит в другой чат (получателю выдачи) —
-- исходного сообщения в этом чате нет, привязка остаётся пустой.
