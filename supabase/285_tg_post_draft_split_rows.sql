-- Замечания ВГ от 26.07.2026 по «Входящим»: нельзя поменять статью у заявки из
-- чата и нельзя разбить одну сумму на несколько статей («купил овощи и мешок
-- цемента одним чеком»).
--
-- Обе задачи решает одна механика — строки. Одна строка с другой статьёй это
-- «поменять статью», несколько строк — «разбить». Отдельной кнопки правки
-- статьи не заводим, чтобы не было двух путей к одному результату.
--
-- Разбивать можно только расход: у перевода статьи нет вовсе.

CREATE OR REPLACE FUNCTION public.tg_post_draft(p_id uuid, p_rows jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  IF v_d.kind = 'expense' THEN
    ...
    ELSE
      SELECT jsonb_agg(jsonb_build_object(
               ...
               'category_id', (r->>'category_id')::uuid) ORDER BY ord)
        INTO v_rows
...
-- Старая односоставная форма остаётся: её зовёт существующий интерфейс.
CREATE OR REPLACE FUNCTION public.tg_post_draft(p_id uuid)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$ SELECT tg_post_draft(p_id, NULL::jsonb); $function$;

REVOKE ALL ON FUNCTION public.tg_post_draft(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.tg_post_draft(uuid, jsonb) TO authenticated;

COMMENT ON FUNCTION public.tg_post_draft(uuid, jsonb) IS
  'Провести заявку из чата. p_rows — разбивка расхода по статьям [{amount, category_id}]; сумма строк обязана сойтись с суммой заявки. NULL — одной строкой, как раньше.';
