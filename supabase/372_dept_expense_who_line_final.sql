-- tg_escape() приводит NULL к пустой строке, поэтому COALESCE снаружи никогда не срабатывал:
-- без автора получалось «Оплатил: «Завод» ()», а в старой версии — висящее «Потратил: ».
-- Решаем на входе: проверяем значение до экранирования.
