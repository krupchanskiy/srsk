-- Карточка-диалог теперь удаляется сразу после решения (✅/✖️) — правка на
-- стороне tg-webhook/index.ts (ВГ 16.09.2026): её работа сделана, статус
-- «ждёт проведения» и так виден по реакции 👀 на исходном сообщении, а сама
-- карточка после решения только занимает место.
--
-- Из-за этого финальное подтверждение («Проведено», «Выдано», «Проведён
-- расход») больше не может отвечать на card_message_id — этого сообщения
-- уже не существует, и Telegram откажет в отправке («message to reply not
-- found»). Отвечаем на source_message_id, как и «Заявка не проведена»
-- (450) — на исходное сообщение автора, которое никогда не удаляется.
