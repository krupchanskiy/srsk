-- Прасад — вторым юнитом внутри ретрита (ВГ, сен 2026): в отличие от кафе
-- (своя касса-счёт), прасад не имеет отдельного счёта — он определяется
-- статьями («Прасад - пожертвование» на приход; «Прасад» и «Закупка
-- готового Прасада» на расход), деньги лежат в тех же счетах, что и обычный
-- денежный поток ретрита. Нужен отдельно, чтобы считать себестоимость и
-- окупаемость прасада.
--
-- «Закупка готового Прасада» сейчас на 100% проходит через кассу кафе —
-- чтобы не задвоить эту сумму (кафе — по счёту, прасад — по статье),
-- прасад-статьи исключены из блока cafe.expense_by_category. На totals
-- (общий итог ретрита) это не влияет — там как считались все проводки
-- объекта, так и считаются.
...
BEGIN
  ...
  -- Прасад-статьи исключены (см. комментарий выше) — они в блоке prasad.
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'category_id', x.category_id, 'name', x.name,
           'base_total', x.base_total, 'by_currency', x.by_currency)
           ORDER BY x.base_total DESC), '[]'::jsonb),
  ...
  -- Прасад отдельным блоком — по статьям, а не по счёту (своей кассы нет).
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'category_id', x.category_id, 'name', x.name,
           'base_total', x.base_total, 'by_currency', x.by_currency)
           ORDER BY x.base_total DESC), '[]'::jsonb),
  ...
  FROM (
    ...
    FROM (
      ...
      FROM fin_postings p
      JOIN fin_categories c ON c.id = p.category_id
      WHERE p.object_id = p_object AND c.direction = 'in' AND c.name = 'Прасад - пожертвование'
      GROUP BY p.category_id, c.name, p.currency_code
    ) s GROUP BY s.category_id, s.name
  ) x;
