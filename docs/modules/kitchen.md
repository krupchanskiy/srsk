...
### Скрытый раздел AB Kitchen

- Входная ссылка: `https://in.rupaseva.com/ab-kitchen/`
- Использует обычную авторизацию и производственную базу BackOffice.
- Входная страница включает в `sessionStorage` контекст `srsk_ab_kitchen_context`.
- В этом контексте `Layout` фиксирует локацию `ab-kitchen` и не показывает другие
  кухни или модули в переключателе.
- В обычной навигации локация `ab-kitchen` отфильтрована и нигде не рекламируется.
- Раздел «Команда» исключён, потому что он использует общий профиль BackOffice.
- `auth-check.js` разрешает в AB-контексте только маршруты `kitchen/` и `stock/` из
  явного белого списка; общий профиль, главная и остальные модули возвращают на
  `/ab-kitchen/` до загрузки защищённой страницы.
- Общая главная страница не удаляет скрытый контекст, а возвращает в AB Kitchen.
- Операционные данные с `location_id` загружаются и изменяются только для локации
  `ab-kitchen`; прямой `id` рецепта другой кухни не открывается в просмотре или редакторе.
