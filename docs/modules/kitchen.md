...
AB Kitchen перенесён в отдельное приложение macOS и больше не имеет точки входа
на публичном сайте BackOffice. Локация `ab-kitchen` остаётся в общей базе данных,
но исключена из веб-переключателя кухонь. Серверные роли и RLS сохраняются для
доступа desktop-приложения.
