...
|--------|----------|------|----------|
| `lead` | Заявка | Синий | Входящая заявка |
| `contacted` | Связались | Фиолетовый | Первый контакт |
| `paid` | Оплачено | Янтарный | Получена оплата |
| `ready` | Готов | Зелёный | Всё готово к заезду |
| `completed` | Завершена | Индиго | Ретрит завершён |
| `cancelled` | Отменена | Красный | Отмена |
...
// → ['paid', 'ready', 'completed', 'cancelled', 'lead']
```
