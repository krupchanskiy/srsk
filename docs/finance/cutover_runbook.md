...
## ✅ ПЕРЕЕЗД СОСТОЯЛСЯ 04.08.2026

Система работает в боевом режиме. Как прошло:

| Шаг | Результат |
|-----|-----------|
| Стоп-кран | включён на время переноса, снят после сверки |
| Снимок | 1 570 строк (`cutover_snapshot`), откат был возможен на всём протяжении |
| Сброс | 222 операции, 306 проводок, 115 заявок стёрты; справочники целы (36 счетов, 11 департаментов, 7 объектов, 7 привязок чатов) |
| Журнал | перезапущен с №1 |
| Дата отсечки | 2026-08-04 |
| Остатки по счетам | 7 opening-проводок, **12 452 051 ₹** под ответственностью |
| Авансы участников | 87 позиций, 86 человек, **1 193 810,05 ₹** (`fin_load_crm_advances`) |
| Сверка ВГ | сошлось |

**Решения, принятые по ходу:**

- *Минус в рублёвой кассе.* ВГ дал −137 221 ₽ — это 700 000 ₽ наличных минус
  долг ашрама перед Ачинтьей Кришной 837 221 ₽. Система запрещает отрицательный
  стартовый остаток наличного счёта (`negative_cash_opening_forbidden`), и это
  правильно: наличных не бывает минус. Оформили раздельно — касса 700 000 ₽ (факт
  в сейфе), новый подотчётный счёт «Ачинтья Кришна (₽)» с остатком −837 221 ₽
  (минус на подотчёте = ашрам должен человеку). Чистая позиция та же, но долг
  виден отдельной строкой.
- *Деньги департаментов.* Суммы, переданные ВГ, **уже включают** средства
  департаментов, поэтому opening по подотчётам не заряжали — иначе двойной счёт.
  ВГ раздаёт их переводами из кассы уже в боевом режиме.
- *Три осиротевших файла* в `finance-files` (метаданные стёрты) снимает ночная
  `fin-cleanup`; прямое удаление из `storage.objects` запрещено политикой Storage.

**Снимок `cutover_snapshot` остаётся в базе как страховка.** Удалить, когда
пройдут три чистых ежедневных чекпоинта (критерий успеха ниже):
`DROP SCHEMA cutover_snapshot CASCADE;`
